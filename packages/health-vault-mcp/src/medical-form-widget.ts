...
function renderReview(preview,out){
  ...
  document.getElementById('app').innerHTML='<div class="hero"><p class="eyebrow">Review before saving</p><h1>'+esc(preview.templateTitle)+'</h1></div><div class="body">'+(preview.reviewFields||[]).map((field)=>'<div class="row"><span class="label">'+esc(field.label)+'</span><div class="value">'+esc(field.value)+'</div></div>').join('')+'</div><div class="notice">'+notice+'</div><div class="actions" style="padding:0 20px 20px"><button class="primary" id="confirm">Confirm &amp; Save</button><a class="secondary" href="https://healthvault.me/?app=medical-forms&form='+encodeURIComponent(preview.templateId)+'&source=chatgpt" id="open">Open secure form</a></div><div id="error"></div>';
