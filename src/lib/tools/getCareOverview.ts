...
export async function getCareOverview(input: unknown) {
  ...
  try {
    ...
    const [
      ...
      supabase.from("health_record_requests").select("id", { count: "exact", head: true }).eq("user_id", userId).eq("status", "sent"),
      supabase.from("health_record_requests").select("id", { count: "exact", head: true }).eq("user_id", userId).eq("status", "received"),
    ...
    return {
      ...
      data: {
        ...
        recordRequestsAwaitingProvider: recordReqSentRes.count || 0,
        recordRequestsCompleted: recordReqReceivedRes.count || 0,
