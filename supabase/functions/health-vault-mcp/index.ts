...
function createHealthVaultMcpServer(supabase: SupabaseClient, userId: string): McpServer {
  ...
  server.registerTool(
    "preview_medical_form_email_share",
    {
      ...
      description: "Use exactly once when all share details are known. Preview one completed form, exact recipient email, expiration, note, and patient-receipt choice. It creates no link or email. Do not call onboarding, dashboard, progress, or recipient-resolution tools first. Do not retry with guessed values after an error. The card is the complete review; do not duplicate it in prose or ask for typed confirmation.",
  ...
  server.registerTool(
    "create_medical_form_email_share",
    {
      ...
      description: "Call exactly once after explicit confirmation of a medical-form share preview, using its exact values. Do not call read tools or preview again. Create and email a scoped, expiring read-only link for that one completed form.",
