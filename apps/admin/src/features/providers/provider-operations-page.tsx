...
import { AlertTriangle, ArrowDown, ArrowUp, ArrowUpDown, Building2, CheckCircle2, Database, ExternalLink, Info, KeyRound, RefreshCw, Search, Send, ShieldAlert, ShieldCheck, Stethoscope, Users, X } from 'lucide-react';
...
function MfaRecoveryView({ canManage }: { canManage: boolean }) {
  ...
  return <div className="provider-view"><div className="mfa-recovery-layout"><section className="provider-table-panel"><div className="provider-panel-heading"><div><h2>MFA recovery</h2><p>Platform-owner recovery for patients, practitioners, provider administrators, and other Health Vault identities.</p></div><span>Fresh MFA required</span></div><div className="mfa-account-lookup"><label className="provider-search"><Search size={17} aria-hidden="true" /><span>Email</span><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="user@example.com" /></label><button type="button" onClick={() => void lookup()} disabled={busy || !canManage || !/^\S+@\S+\.\S+$/.test(email)}>{busy ? 'Searching…' : 'Find account'}</button></div>{error && <div className="provider-action-note"><AlertTriangle size={17} /><p>{error}</p></div>}{message && <div className="provider-action-note"><ShieldCheck size={17} /><p>{message}</p></div>}</section><aside className="mfa-guidance-card" aria-labelledby="mfa-guidance-title"><div className="mfa-guidance-heading"><span><Info size={18} /></span><div><h2 id="mfa-guidance-title">Before resetting MFA</h2><p>This is an account-recovery action, not a routine access change.</p></div></div><ul><li><strong>Verify the request.</strong> Confirm the support request and the user’s identity before removing a factor.</li><li><strong>Expect sign-out.</strong> The reset invalidates the user’s existing sessions across Health Vault.</li><li><strong>Plan re-enrollment.</strong> The user must enroll a new authenticator before returning to protected actions.</li><li><strong>Protect the patient profile.</strong> Resetting MFA does not transfer ownership or delete profile data.</li></ul><p className="mfa-guidance-footnote"><KeyRound size={15} /> Your own recent MFA verification is required before a reset can be completed.</p></aside></div>{account && <section className="provider-table-panel"><div className="provider-panel-heading"><div><h2>Reset authenticator for {account.email}</h2><p>{account.verifiedFactors.length} verified factor{account.verifiedFactors.length === 1 ? '' : 's'} will be removed. This cannot be undone.</p></div><KeyRound /></div><div className="provider-toolbar"><label className="provider-search"><span>Type the account email to confirm</span><input value={confirmEmail} onChange={(event) => setConfirmEmail(event.target.value)} placeholder={account.email} /></label><label className="provider-search"><span>Required recovery reason</span><input value={reason} onChange={(event) => setReason(event.target.value)} maxLength={500} placeholder="Document identity verification and support ticket" /></label><button type="button" onClick={() => void reset()} disabled={busy || !canManage || confirmEmail.trim().toLowerCase() !== account.email.toLowerCase() || reason.trim().length < 10}><KeyRound size={16} />{busy ? 'Resetting…' : 'Reset MFA and sessions'}</button><button type="button" onClick={() => { setAccount(null); setConfirmEmail(''); setReason(''); }} disabled={busy}>Cancel</button></div></section>}</div>;
}
